...
public class DefaultMetaCache implements IMetaCache {
  ...
  private final Map<ColumnsInterval, ColumnsIntervalStatistics> activeColumnsIntervalStatisticsMap;
  ...
  public double getMinActiveIginxStatistics() {
    return activeIginxStatisticsMap.values().stream()
  ...
  public void addOrUpdateActiveColumnStatistics(Map<String, ColumnStatistics> statisticsMap) {
    ...
    statisticsMap.forEach(
        (k, v) ->
            activeColumnStatisticsMap.computeIfAbsent(k, e -> new ColumnStatistics()).update(v));
    ...
    for (ColumnStatistics statistics : statisticsMap.values()) {
      ...
      if (activeStorageEngineStatisticsMap.containsKey(storageEngineId)) {
        ...
        activeStorageEngineStatisticsMap.put(
            storageEngineId,
            new StorageEngineStatistics(statistics.getTotalHeat(), 1.0, statistics.getTotalHeat()));
      }
    }
  }
  ...
  public Map<ColumnsInterval, ColumnsIntervalStatistics>
    ...
    Map<ColumnsInterval, ColumnsIntervalStatistics> columnsIntervalStatisticsMap = new TreeMap<>();
    ...
    for (String separator : separators) {
      ...
      columnsIntervalStatisticsMap.put(
          new ColumnsInterval(prevColumn, nextColumn), new ColumnsIntervalStatistics(tempSum));
