...
public class FilterTransformer {
  ...
  private static String toString(KeyFilter filter) {
    String op = Op.op2Str(filter.getOp()).replace("==", "="); // postgresql does not support "==" but uses "=" instead
    return (KEY_NAME + " " + op + " " + filter.getValue()).replace(IGINX_SEPARATOR, Constants.POSTGRESQL_SEPARATOR);
  }

  private static String toString(ValueFilter filter) {
    int lastIndexOfSeparator = filter.getPath().lastIndexOf(IGINX_SEPARATOR);
    String path = filter.getPath().substring(0, lastIndexOfSeparator).replace(IGINX_SEPARATOR, Constants.POSTGRESQL_SEPARATOR)
            + filter.getPath().substring(lastIndexOfSeparator);

    String op = filter.getOp() == Op.LIKE
            ? "~"
            : Op.op2Str(filter.getOp()).replace("==", "="); // postgresql does not support "==" but uses "=" instead

    String regex_symbol = filter.getOp() == Op.LIKE ? "$" : "";

    Object value = filter.getValue().getDataType() == DataType.BINARY
  ...
  private static String toString(PathFilter filter) {
    ...
    String pathA = filter.getPathA().substring(0, lastIndexOfSeparatorA).replace(IGINX_SEPARATOR, Constants.POSTGRESQL_SEPARATOR)
            + filter.getPathA().substring(lastIndexOfSeparatorA);
    String pathB = filter.getPathB().substring(0, lastIndexOfSeparatorB).replace(IGINX_SEPARATOR, Constants.POSTGRESQL_SEPARATOR)
