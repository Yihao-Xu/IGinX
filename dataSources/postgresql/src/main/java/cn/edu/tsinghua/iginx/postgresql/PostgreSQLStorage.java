...
public class PostgreSQLStorage implements IStorage {
  ...
  private TaskExecuteResult executeProjectWithFilter(Project project, Filter filter, DataArea dataArea) {
    try {
      ...
      filter = queryWithFilter(filter, databaseNameList, resultSets, conn, databaseName, tableNameToColumnNames);
  ...
  private Filter queryWithFilter(Filter filter, List<String> databaseNameList, List<ResultSet> resultSets, Connection conn, String databaseName, Map<String, String> tableNameToColumnNames) {
    String statement;
    Statement stmt;
    ...
    //table中带有了通配符，将所有table都join到一起进行查询，以便输入filter.
      ...
      for (int i = 1; i < tableNames.size(); i++){
        ...
        for(int j = 0; j < i; j++){
          fullTableName.append(tableNames.get(i)).append(IGINX_SEPARATOR).append(KEY_NAME)
                  .append(" = ")
                  .append(tableNames.get(j)).append(IGINX_SEPARATOR).append(KEY_NAME);
          if (j != i - 1){
